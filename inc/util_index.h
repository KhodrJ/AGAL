...
)
{
    ...
    *nbr_kap_c = Ip + 4*Jp + 16*Kp;
    ...
    {
        *halo_change = 
            (Ip != I+incr*V_CONN_ID[p + 0*27] || V_CONN_ID[p + 0*27]==0) && 
            (Jp != J+incr*V_CONN_ID[p + 1*27] || V_CONN_ID[p + 1*27]==0) && 
            (Kp != K+incr*V_CONN_ID[p + 2*27] || V_CONN_ID[p + 2*27]==0);
    }
...
__device__ __forceinline__
int Cu_FillHaloWithNbrs
(
    const int &p,
    T *s_u,
    const T *__restrict__ g_u,
    const int &I,
    const int &J,
    const int &K,
    int *__restrict__ s_ID_nbr,
...
{
    // Retrieve data from global memory g_u and fill up one cell-layer around the current block from neighbor blocks in a specific direction.
    // Data goes into shared memory array s_u.
    // Note: Does NOT fill the current block, do it manually.
    ...
    // Load data into the halo if these cells participate.
    if (changed && nbr_kap_b > -1)
        s_u[nbr_kap_h] = g_u[nbr_kap_b*block_size + nbr_kap_c + offset];
