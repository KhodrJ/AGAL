...
class Parser
{
    ...
    int PrintMaps()
    {
        for (std::map<std::string, int>::iterator it = params_int.begin(); it != params_int.end(); ++it)
            std::cout << it->first << " " << it->second << std::endl;
        for (std::map<std::string, double>::iterator it = params_dbl.begin(); it != params_dbl.end(); ++it)
            std::cout << it->first << " " << it->second << std::endl;
        for (std::map<std::string, std::string>::iterator it = params_str.begin(); it != params_str.end(); ++it)
            std::cout << it->first << " " << it->second << std::endl;
    ...
    int ReadInputFile()
    {
        ...
        params_int = std::map<std::string, int> // Define map of integer parameters with default values.
        {
            {"MAX_LEVELS_WALL",         5},
        ...
        params_dbl = std::map<std::string, double> // Define map of double parameters with default values.
        {
            {"L_c",                     1.0},
            {"L_fy",                    1.0},
            {"L_fz",                    1.0},
        ...
        params_str = std::map<std::string, std::string> // Define map of string parameters with default values.
        {
        ...
        for (std::string line; std::getline(input, line);) 
        {
            if (line.length() > 0 && line.at(0) != '#') // Ignore comments.
            {
                std::stringstream ss(line);
                ss >> str_name >> str_val;
                if ( params_int.count(str_name) == 1 )
                    params_int[str_name] = ParseNxFactor(str_val, Nx);
                else if ( params_dbl.count(str_name) == 1 )
                    params_dbl[str_name] = std::stod(str_val);
                else if ( params_str.count(str_name) == 1 )
                    params_str[str_name] = str_val;
                else
    ...
    Parser(std::string input_file_directory_) : input_file_directory(input_file_directory_)
    {
        ReadInputFile();
    }
};
