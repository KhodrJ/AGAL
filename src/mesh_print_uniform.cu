...
int Mesh::M_RenderAndPrint_Uniform(int i_dev, int iter, int *params)
{
	...
	*/
	...
	grid->SetDimensions(Nxi_f[0]+1, Nxi_f[1]+1, N_DIM==1?2:Nxi_f[2]+1);
...
// 		vtkNew<vtkImageMapToColors> scalar_values_to_colors;
// 		scalar_values_to_colors->SetLookupTable(lookup_table);
// 		scalar_values_to_colors->PassAlphaToOutputOn();
// 			scalar_values_to_colors->SetInputData(cell_to_points->GetImageDataOutput());
// 		vtkNew<vtkImageActor> actor;
// 		actor->GetMapper()->SetInputConnection(scalar_values_to_colors->GetOutputPort(0));
		...
		std::string photo_name = output_dir + std::string("img/shot_") + padded_iter + ".png";
	...
	if ((iter+1)%P_PRINT == 0)
	{
		...
		std::string file_name = output_dir + std::string("out_") + std::to_string(iter+1) + ".vti";
