...
% Open files.
fileID_d2q9 = fopen("./out/solver_lbm_average_d2q9.cu",'w');
fileID_d3q19 = fopen("./out/solver_lbm_average_d3q19.cu",'w');
fileID_d3q27 = fopen("./out/solver_lbm_average_d3q27.cu",'w');
...
for K = 1:3
	...
	% Build kernel header.
	args_1 = {"int n_ids_idev_L", "int *id_set_idev_L", "long int n_maxcells", "ufloat_t dx_Lm1", "ufloat_t dx_L", "ufloat_t tau_L", "ufloat_t tau_ratio", "ufloat_t v0"};
	...
	args_1_1 = {"mesh->n_ids[i_dev][L]", "&mesh->c_id_set[i_dev][L*n_maxcblocks]", "n_maxcells", "mesh->dxf_vec[L+1]", "mesh->dxf_vec[L]", "tau_L", "tau_ratio_L", "v0"};
	...
	variation_1 = ...
	{...
		'condition', "mesh->n_ids[i_dev][L] > 0 && var == V_AVERAGE_INTERFACE",...
		'params', "(M_CBLOCK+mesh->n_ids[i_dev][L]-1)/M_CBLOCK,M_CBLOCK,0,mesh->streams[i_dev]",...
		'args', args_kernel_1,...
		'template', "<0>"...
	};
	args_2_1 = {"mesh->n_ids[i_dev][L]", "&mesh->c_id_set[i_dev][L*n_maxcblocks]", "n_maxcells", "mesh->dxf_vec[L+1]", "mesh->dxf_vec[L]", "tau_L", "tau_ratio_L", "v0"};
	args_2_2 = {"mesh->c_cblock_ID_onb[i_dev]", "mesh->c_cblock_ID_mask[i_dev]", "mesh->c_cblock_ID_nbr_child[i_dev]", "n_maxcblocks"};
	args_2_3 = {"mesh->c_cells_ID_mask[i_dev]", "mesh->c_cells_f_F[i_dev]"};
	args_kernel_2 = {args_2_1, args_2_2, args_2_3};
	variation_2 = ...
	{...
		'condition', "mesh->n_ids[i_dev][L] > 0 && var == V_AVERAGE_BLOCK",...
		'params', "(M_CBLOCK+mesh->n_ids[i_dev][L]-1)/M_CBLOCK,M_CBLOCK,0,mesh->streams[i_dev]",...
		'args', args_kernel_2,...
		'template', "<1>"...
	};
	args_3_1 = {"mesh->n_ids[i_dev][L]", "&mesh->c_id_set[i_dev][L*n_maxcblocks]", "n_maxcells", "mesh->dxf_vec[L+1]", "mesh->dxf_vec[L]", "tau_L", "tau_ratio_L", "v0"};
	args_3_2 = {"mesh->c_cblock_ID_onb[i_dev]", "mesh->c_cblock_ID_mask[i_dev]", "mesh->c_cblock_ID_nbr_child[i_dev]", "n_maxcblocks"};
	args_3_3 = {"mesh->c_cells_ID_mask[i_dev]", "mesh->c_cells_f_F[i_dev]"};
	args_kernel_3 = {args_3_1, args_3_2, args_3_3};
	variation_3 = ...
	{...
		'condition', "mesh->n_ids[i_dev][L] > 0 && var == V_AVERAGE_GRID",...
		'params', "(M_CBLOCK+mesh->n_ids[i_dev][L]-1)/M_CBLOCK,M_CBLOCK,0,mesh->streams[i_dev]",...
		'args', args_kernel_3,...
		'template', "<2>"...
	};
	...
	%
	build_kernel_routine(fileID{K}, 0, "Solver_LBM::S", sprintf("Average_%s", l_dq_names{K}), args_routine, variations);
	add_line(fileID{K});
...
r = system("cp ./out/solver_lbm_average_* ../");
if (r==0)
