...
)
{
	...
	for (int k = 0; k < M_CBLOCK; k++)
	{
		...
		if ( i_kap_b > -1 && (i_kap_bc < 0 || block_on_boundary == 1) )
		{
			...
			int nbr_id_19 = cblock_ID_nbr[i_kap_b + 19*n_maxcblocks];
			...
			__syncthreads();
			s_F_p[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 19*n_maxcells];
			s_F_pb[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 20*n_maxcells];
				// nbr 19 (p = 20)
				// This nbr participates in a regular streaming exchange.
			{
				nbr_kap_b = nbr_id_19;
			...
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 19*n_maxcells] = s_F_pb[(I_kap + (1) + 1) + (Nbx+2)*(J_kap + (1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (1) + 1)];
			}
			if (s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)] >= 0)
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 20*n_maxcells] = s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)];
			}
			...
			__syncthreads();
			s_F_p[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 21*n_maxcells];
			s_F_pb[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 22*n_maxcells];
				// nbr 5 (p = 21)
				// This nbr participates in a regular streaming exchange.
			{
				nbr_kap_b = nbr_id_5;
			...
			if (s_F_pb[(I_kap + (1) + 1) + (Nbx+2)*(J_kap + (1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)] >= 0)
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 21*n_maxcells] = s_F_pb[(I_kap + (1) + 1) + (Nbx+2)*(J_kap + (1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)];
			}
			if (s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (1) + 1)] >= 0)
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 22*n_maxcells] = s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (1) + 1)];
			}
			...
			__syncthreads();
			s_F_p[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 23*n_maxcells];
			s_F_pb[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 24*n_maxcells];
				// nbr 3 (p = 23)
				// This nbr participates in a regular streaming exchange.
			{
				nbr_kap_b = nbr_id_3;
			...
			if (s_F_pb[(I_kap + (1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (1) + 1)] >= 0)
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 23*n_maxcells] = s_F_pb[(I_kap + (1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (1) + 1)];
			}
			if (s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)] >= 0)
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 24*n_maxcells] = s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)];
			}
			...
			__syncthreads();
			s_F_p[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 26*n_maxcells];
			s_F_pb[(I_kap+1)+(Nbx+2)*(J_kap+1)+(Nbx+2)*(Nbx+2)*(K_kap+1)] = cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 25*n_maxcells];
				// nbr 11 (p = 26)
				// This nbr participates in a regular streaming exchange.
			{
				nbr_kap_b = nbr_id_11;
			...
			if (s_F_pb[(I_kap + (1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)] >= 0)
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 26*n_maxcells] = s_F_pb[(I_kap + (1) + 1) + (Nbx+2)*(J_kap + (-1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (-1) + 1)];
			}
			if (s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (1) + 1)] >= 0)
			{
				cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 25*n_maxcells] = s_F_p[(I_kap + (-1) + 1) + (Nbx+2)*(J_kap + (1) + 1) + (Nbx+2)*(Nbx+2)*(K_kap + (1) + 1)];
			}
...
int Solver_LBM::S_Stream_Inpl_d3q27(int i_dev, int L)
{
	...
	{
		Cu_Stream_Inpl_d3q27<<<(M_CBLOCK+mesh->n_ids[i_dev][L]-1)/M_CBLOCK,M_CBLOCK,0,mesh->streams[i_dev]>>>
		(
