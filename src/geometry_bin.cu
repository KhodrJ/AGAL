...
)
{
	...
	if (kap < n_faces)
	{
		...
		for (int j = j0; j < j0+G_BIN_NUM; j++)
		{
			...
			ufloat_g_t xm = (bi - rad)*LxOg;
			ufloat_g_t xM = (bi+1 + rad)*LxOg;
			ufloat_g_t ym = (bj - rad)*LyOg;
			ufloat_g_t yM = (bj+1 + rad)*LyOg;
#if (N_DIM==3)
			ufloat_g_t zm = (bk - rad)*LzOg;
			ufloat_g_t zM = (bk+1 + rad)*LzOg;
#endif
...
int Geometry::G_MakeBins(int i_dev)
{
...
#if (N_DIM==2)
	ufloat_g_t minL0g = std::min({Lx0g,Lx0g,Lx0g});
#else
	ufloat_g_t minL0g = std::min({Lx0g,Lx0g,Lx0g});
#endif
	...
	cudaDeviceSynchronize();
	std::cout << "Elapsed time: " << toc_simple("",T_MS,0) << std::endl;
