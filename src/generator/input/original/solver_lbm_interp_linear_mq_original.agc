...
OUTFOR i_qj_q 3   0 Nqx/2 1   0 Nqx/2 1
    INFOR I_qJ_qK_q 3   0 2 1   0 2 1   0 gI(Ldim-1) 1
        ...
        INFOR p 1   0 Lsize 1
            ...
            //
            INIF Ldim==2
                REG-vz cdotu = gNz(Lc0(<p>)*u + Lc1(<p>)*v);
            INELSE
            ...
            REG __syncthreads();
            INFOR xc_ixc_jxc_k 4   0 2 1   0 2 1   0 gI(Ldim-1) 1
                REG-m //\tChild quadrant gI(<xc_i>+2*<xc_j>+4*<xc_k>).
                INIF Ldim==2
                    REG i_Qcp = i_Qc + (<xc_i>+Nqx*<xc_j>);
                INELSE
                    REG i_Qcp = i_Qc + (<xc_i>+Nqx*<xc_j>+Nqx*Nqx*<xc_k>);
                END_INIF
                OUTIF ((interp_type == 0 && cells_ID_mask[(i_kap_bc+gI(<I_q>+2*<J_q>+4*<K_q>))*M_CBLOCK + i_Qcp*M_TBLOCK + threadIdx.x]==2) || (interp_type == 1))
                    #DEFINE q gI(2*<xc_i> + 2*4*<xc_j> + 2*4*4*<xc_k>)
                    
                    INIF Ldim==2
                        REG-tvm cells_f_F[(i_kap_bc+gI(<I_q>+2*<J_q>+4*<K_q>))*M_CBLOCK + i_Qcp*M_TBLOCK + threadIdx.x + Lpb(<p>)*n_maxcells] = \
                        s_F[gI(0+sQ)] + \
