...
OUTIFL (geometry_init && valid_mask == -2)
    REG valid_block = cblock_ID_onb_solid[i_kap_b];
END_OUTIFL
<
...
INFOR p 1   1 Lsize 1
    INIF ( (Ldim==2 and (<p>==1 or (<p>+1)%3==0)) or (Ldim==3 and ((((<p>-1)%2==0)and(<p><25))or(<p>==26))) )
        ...
        OUTIFL (valid_mask == -2)
            REG dQ = cells_f_X_b[valid_block*M_CBLOCK + threadIdx.x + <p>*n_maxcells_b];
        END_OUTIFL
        <
        ...
        OUTIF ( valid_mask != -1 && F_pb>=0 && dQ < 0)
