...
NAME_FILE solver_lbm_interp_linear_mq
INCLUDE "solver.h"
INCLUDE "mesh.h"
...
LOOPBLOCKS 2\
    ...
    OUTFOR i_qj_q 3   0 Nqx/2 1   0 Nqx/2 1
        INFOR I_qJ_qK_q 3   0 2 1   0 2 1   0 ^I< LBM_dim-1 >^ 1
            ...
            #REG load_and_calc = false;
            INFOR p 1   0 LBM_size 1
                //
                // DDF <p>.
                //
                INIF LBM_dim==2
                    REG cdotu = LBM_c0(<p>)*u_kap + LBM_c1(<p>)*v_kap;
                INELSE
                    ...
                    OUTIF (interp_type == 0 && cells_ID_mask[(i_kap_bc+IKAPC)*M_CBLOCK + i_Qcp*M_TBLOCK + threadIdx.x]==2) || (interp_type == 1)
                        ...
                        #OUTIF !load_and_calc
                            ...
                            #INELSE
                                ...
                                #REG udotu = u_kap*u_kap + v_kap*v_kap + w_kap*w_kap;
                            #END_INIF
                            #REG load_and_calc = true;
                        #END_OUTIF
                        
                        DEFINE q ^I<2*<xc_i> + 2*4*<xc_j> + 2*4*4*<xc_k> >^
                        INIF LBM_dim==2
                            REG cells_f_F[(i_kap_bc+IKAPC)*M_CBLOCK + i_Qcp*M_TBLOCK + threadIdx.x + LBM_pb(<p>)*n_maxcells] = \
                            REG s_F[^I<0+q>^] + \
