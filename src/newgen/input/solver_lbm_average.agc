...
OUTFOR xc_ixc_j 4   0 2 1   0 2 1
    ...
    INFOR p 1   0 LBM_size 1
        REG f_<p> = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + LBM_pb(<p>)*n_maxcells];
    END_INFOR
    ...
    REG __syncthreads();
        ...
        INFOR p 1   0 LBM_size 1
            ...
            END_INIF
                ...
                INIF LBM_dim==2
                    REG cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + LBM_pb(<p>)*n_maxcells] = N_Pf(0.25)*( s_Fc[(child0_IJK + 0 + Nbx*0)] + s_Fc[(child0_IJK + 1 + Nbx*0)] + s_Fc[(child0_IJK + 0 + Nbx*1)] + s_Fc[(child0_IJK + 1 + Nbx*1)] );
                INELSE
                    REG cells_f_F[i_kap_b*M_CBLOCK  + threadIdx.x + LBM_pb(<p>)*n_maxcells] = N_Pf(0.125)*( s_Fc[(child0_IJK + 0 + Nbx*0 + Nbx*Nbx*0)] + s_Fc[(child0_IJK + 1 + Nbx*0 + Nbx*Nbx*0)] + s_Fc[(child0_IJK + 0 + Nbx*1 + Nbx*Nbx*0)] + s_Fc[(child0_IJK + 1 + Nbx*1 + Nbx*Nbx*0)] + s_Fc[(child0_IJK + 0 + Nbx*0 + Nbx*Nbx*1)] + s_Fc[(child0_IJK + 1 + Nbx*0 + Nbx*Nbx*1)] + s_Fc[(child0_IJK + 0 + Nbx*1 + Nbx*Nbx*1)] + s_Fc[(child0_IJK + 1 + Nbx*1 + Nbx*Nbx*1)] );
                END_INIF
