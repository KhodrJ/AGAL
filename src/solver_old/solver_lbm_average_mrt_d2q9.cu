		...
		if (i_kap_b > -1 && ((i_kap_bc>-1)and((ave_type==2)or(block_on_interface==1))))
		{
			for (int xc_j = 0; xc_j < 2; xc_j += 1)
			{
				for (int xc_i = 0; xc_i < 2; xc_i += 1)
				{
					...
					m_1 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 3*n_maxcells];
					m_2 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 4*n_maxcells];
					m_3 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 1*n_maxcells];
					m_4 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 2*n_maxcells];
					m_5 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 7*n_maxcells];
					m_6 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 8*n_maxcells];
					m_7 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 5*n_maxcells];
					m_8 = cells_f_F[(i_kap_bc + xc)*M_CBLOCK + threadIdx.x + 6*n_maxcells];
					rho_kap = m_0;
					u_kap = m_3 / m_0;
					v_kap = m_5 / m_0;
...
int Solver_LBM::S_Average_MRT_d2q9(int i_dev, int L, int var, ufloat_t tau_L, ufloat_t tau_ratio_L)
{
	...
	{
		Cu_Average_MRT_d2q9<2><<<(M_LBLOCK+mesh->n_ids[i_dev][L]-1)/M_LBLOCK,M_TBLOCK,0,mesh->streams[i_dev]>>>(mesh->n_ids[i_dev][L], n_maxcells, n_maxcblocks, tau_L, tau_ratio_L, &mesh->c_id_set[i_dev][L*n_maxcblocks], mesh->c_cells_ID_mask[i_dev], mesh->c_cells_f_F[i_dev], mesh->c_cblock_ID_nbr[i_dev], mesh->c_cblock_ID_nbr_child[i_dev], mesh->c_cblock_ID_mask[i_dev], mesh->c_cblock_ID_onb[i_dev]);
	}
