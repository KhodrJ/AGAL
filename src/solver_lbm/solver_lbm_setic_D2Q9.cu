...
__global__
void Cu_SetInitialConditions_D2Q9(int n_ids_idev_L,long int n_maxcells,int n_maxcblocks,int *id_set_idev_L,int *cells_ID_mask,ufloat_t *cells_f_F,ufloat_t *cblock_f_X,int *cblock_ID_mask,int *cblock_ID_onb,ufloat_t dx_L)
{
    ...
    for (int k = 0; k < M_LBLOCK; k += 1)
    {
        ...
        if ((i_kap_b>-1))
        {
            ...
            if ( cells_ID_mask[i_kap_b*M_CBLOCK + threadIdx.x] != -1 )
            {
                ...
                udotu = u*u + v*v;
                cdotu = (ufloat_t)(0.0);
                f_0 = rho*(ufloat_t)0.444444444444444*( (ufloat_t)1.0 + (ufloat_t)3.0*cdotu + (ufloat_t)4.5*cdotu*cdotu - (ufloat_t)1.5*udotu );
                cdotu = (u);
                f_1 = rho*(ufloat_t)0.111111111111111*( (ufloat_t)1.0 + (ufloat_t)3.0*cdotu + (ufloat_t)4.5*cdotu*cdotu - (ufloat_t)1.5*udotu );
                cdotu = (v);
                f_2 = rho*(ufloat_t)0.111111111111111*( (ufloat_t)1.0 + (ufloat_t)3.0*cdotu + (ufloat_t)4.5*cdotu*cdotu - (ufloat_t)1.5*udotu );
                cdotu = -(u);
                f_3 = rho*(ufloat_t)0.111111111111111*( (ufloat_t)1.0 + (ufloat_t)3.0*cdotu + (ufloat_t)4.5*cdotu*cdotu - (ufloat_t)1.5*udotu );
                cdotu = -(v);
                f_4 = rho*(ufloat_t)0.111111111111111*( (ufloat_t)1.0 + (ufloat_t)3.0*cdotu + (ufloat_t)4.5*cdotu*cdotu - (ufloat_t)1.5*udotu );
                cdotu = (u+v);
                f_5 = rho*(ufloat_t)0.027777777777778*( (ufloat_t)1.0 + (ufloat_t)3.0*cdotu + (ufloat_t)4.5*cdotu*cdotu - (ufloat_t)1.5*udotu );
                cdotu = (v)-(u);
                f_6 = rho*(ufloat_t)0.027777777777778*( (ufloat_t)1.0 + (ufloat_t)3.0*cdotu + (ufloat_t)4.5*cdotu*cdotu - (ufloat_t)1.5*udotu );
                cdotu = -(u+v);
...
int Solver_LBM<ufloat_t,ufloat_g_t,AP,LP>::S_SetInitialConditions_D2Q9(int i_dev, int L)
{
