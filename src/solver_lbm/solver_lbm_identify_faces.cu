...
)
{
	...
	for (int k = 0; k < M_LBLOCK; k += 1)
	{
		...
		if ((i_kap_b>-1)&&(valid_block>-1))
		{
			...
			ufloat_g_t vxp = cblock_f_X[i_kap_b + 0*n_maxcblocks] + (threadIdx.x%4)*dx_L + (ufloat_g_t)(0.5)*dx_L;
			ufloat_g_t vyp = cblock_f_X[i_kap_b + 1*n_maxcblocks] + ((threadIdx.x/4)%4)*dx_L + (ufloat_g_t)(0.5)*dx_L;
