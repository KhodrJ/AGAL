...
void Cu_StoreFaces
(
	int n_ids_idev_L, int *id_set_idev_L, int n_maxcblocks, int n_maxblocks_b, ufloat_t dx_L,
	int *cells_ID_mask,
	ufloat_t *cblock_f_X, int *cblock_ID_onb_solid, int *cblock_ID_face,
	...
	for (int k = 0; k < M_LBLOCK; k += 1)
	{
		...
		if ((i_kap_b>-1)&&(valid_block>-1))
		{
			...
			for (int p = 0; p < n_f; p++)
			{
				...
				s_D[threadIdx.x] = 0;
				if (add_face && cells_ID_mask[i_kap_b*M_CBLOCK + threadIdx.x] == -2)
					s_D[threadIdx.x] = 1;
				__syncthreads();
				
				// Reduction for the sum (or maximum, same result).
				...
				{
					cblock_ID_face[valid_block + face_counter*n_maxblocks_b] = f_p;
					face_counter++;
					
					if (face_counter == N_Q_max)
						printf("WARNING: Reached face storage limit, increase it to ensure accurate capture of the boundary.\n");
				}
...
// 						printf("Found a curved boundary with d=%17.15f\n", dist_p);
					}
...
// 					if (s_D[threadIdx.x] > 0)
// 						printf("Block %i added face %i\n",i_kap_b,f_p);
// 					
// 				}
...
// 				add_face = false;
			...
			//if (check_cell_mask && cells_ID_mask[i_kap_b*M_CBLOCK + threadIdx.x]==-2)
			//		cells_ID_mask[i_kap_b*M_CBLOCK + threadIdx.x] = -3;
			//if (threadIdx.x==0 && counter>0)
			//	printf("Block %i counted %i faces.\n",i_kap_b,counter);
		}
	}
}
...
int Solver_LBM<ufloat_t,ufloat_g_t,AP,LP>::S_IdentifyFaces(int i_dev, int L)
{
	...
	{
// 		Cu_StoreFaces<ufloat_t,ufloat_g_t,AP,LP->VS> <<<(M_LBLOCK+mesh->n_ids[i_dev][L]-1)/M_LBLOCK,M_TBLOCK,0,mesh->streams[i_dev]>>>(
// 			mesh->n_ids[i_dev][L], &mesh->c_id_set[i_dev][L*n_maxcblocks], n_maxcblocks, mesh->n_solidb, dxf_vec[L],
// 			mesh->c_cells_ID_mask[i_dev],
// 			mesh->c_cblock_f_X[i_dev], mesh->c_cblock_ID_onb_solid[i_dev], mesh->c_cblock_ID_face[i_dev],
