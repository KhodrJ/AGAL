...
void Cu_Collision_New_D2Q9(int n_ids_idev_L,long int n_maxcells,int n_maxcblocks,ufloat_t dx_L,ufloat_t tau_L,int *__restrict__ id_set_idev_L,int *__restrict__ cells_ID_mask,ufloat_t *__restrict__ cells_f_F,ufloat_t *__restrict__ cells_f_F_aux,ufloat_t *__restrict__ cblock_f_X,int *__restrict__ cblock_ID_nbr,int *__restrict__ cblock_ID_nbr_child,int *__restrict__ cblock_ID_mask,int *__restrict__ cblock_ID_onb)
{
    ...
    for (int k = 0; k < M_LBLOCK; k += 1)
    {
        ...
        if ((i_kap_b>-1)&&((i_kap_bc<0)||(valid_block>-3)))
        {
            ...
            u = ( (f1+f5+f8)-(f3+f6+f7) )/rho;
            v = ( (f2+f5+f6)-(f4+f7+f8) )/rho;
...
int Solver_LBM<ufloat_t,ufloat_g_t,AP,LP>::S_Collision_New_D2Q9(int i_dev, int L)
{
	...
	{
		Cu_Collision_New_D2Q9<ufloat_t,ufloat_g_t,AP><<<(M_LWBLOCK+mesh->n_ids[i_dev][L]-1)/M_LWBLOCK,M_WBLOCK*M_LWBLOCK,0,mesh->streams[i_dev]>>>(mesh->n_ids[i_dev][L], n_maxcells, n_maxcblocks, dxf_vec[L], tau_vec[L], &mesh->c_id_set[i_dev][L*n_maxcblocks], mesh->c_cells_ID_mask[i_dev], mesh->c_cells_f_F[i_dev], mesh->c_cells_f_F_aux[i_dev], mesh->c_cblock_f_X[i_dev], mesh->c_cblock_ID_nbr[i_dev], mesh->c_cblock_ID_nbr_child[i_dev], mesh->c_cblock_ID_mask[i_dev], mesh->c_cblock_ID_onb[i_dev]);
	}
