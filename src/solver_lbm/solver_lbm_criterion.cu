...
)
{
    ...
    if (i_kap_b > -1)
    {
        if (cblock_ID_ref[i_kap_b] != V_REF_ID_INACTIVE)
        {
            ...
            BlockwiseReduction<ufloat_t>(threadIdx.x,blockDim.x,s_W);
            __syncthreads();
//         }
//     }
//     
//     Evaluate criterion.
//     if (blockIdx.x < id_max_curr && cblock_ID_ref[kap] != V_REF_ID_INACTIVE)
//     {
...
int Solver_LBM<ufloat_t,ufloat_g_t,AP,LP>::S_ComputeRefCriteria(int i_dev, int L, int var)
{
    ...
    {
        Cu_ComputeRefCriteria<ufloat_t,AP,LP> <<<(M_TBLOCK+mesh->id_max[i_dev][MAX_LEVELS]-1)/M_TBLOCK,M_TBLOCK,0,mesh->streams[i_dev]>>>(
