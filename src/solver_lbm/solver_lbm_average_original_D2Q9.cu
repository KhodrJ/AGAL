        ...
        if ((i_kap_b>-1)&&((i_kap_bc>-1)and((ave_type==2)or(block_on_boundary==1))))
        {
            for (int xc_i = 0; xc_i < 2; xc_i += 1)
            {
                for (int xc_j = 0; xc_j < 2; xc_j += 1)
                {
                    ...
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 0*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 3*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 4*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 1*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 2*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 7*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 8*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 5*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                    ...
                    if ((s_ID_mask_child[child0_IJK] == 1) and (I >= 2*xc_i) and (I < 2+2*xc_i) and (J >= 2*xc_j) and (J < 2+2*xc_j))
                    {
                        cells_f_F[i_kap_b*M_CBLOCK + threadIdx.x + 6*n_maxcells] = (ufloat_t)(0.25)*( s_Fc[(child0_IJK + 0 + 4*0)] + s_Fc[(child0_IJK + 1 + 4*0)] + s_Fc[(child0_IJK + 0 + 4*1)] + s_Fc[(child0_IJK + 1 + 4*1)] );
                    }
                    __syncthreads();
                }
            }
