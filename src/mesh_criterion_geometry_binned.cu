...
{
	int D = (2*N_DIM+1);
	for (int kap = 0; kap < id_max_curr; kap++)
	{
		// Only consider blocks marked for refinement.
		if (cblock_ID_ref[kap] == V_REF_ID_MARK_REFINE)
		{
			for (int d = 1; d < D; d++)
			{
				int i_p = cblock_ID_nbr[kap + d*n_maxcblocks];
...
)
{
	...
	{
		// Only consider blocks marked for refinement.
		if (cblock_ID_ref[kap] == V_REF_ID_INDETERMINATE)
		{
			bool eligible = true;
	...
	if (n_ids[i_dev][L] > 0)
	{
		...
		if (!use_cpu)
		{
			...
			for (int j = 0; j < N_prop; j++)
			{
				...
				Cu_PropagateMarks_S1<<<(M_BLOCK+id_max[i_dev][L]-1)/M_BLOCK,M_BLOCK>>>(
					id_max[i_dev][L], n_maxcblocks,
					c_cblock_ID_ref[i_dev], c_cblock_ID_nbr[i_dev], c_cblock_ID_nbr_child[i_dev]
				);
				Cu_PropagateMarks_S2<<<(M_BLOCK+id_max[i_dev][L]-1)/M_BLOCK,M_BLOCK>>>(
					id_max[i_dev][L], n_maxcblocks,
					c_cblock_ID_ref[i_dev], c_cblock_ID_nbr[i_dev], c_cblock_ID_nbr_child[i_dev]
				);
			}
		...
		else
		{
			...
			for (int j = 0; j < N_prop; j++)
			{
				...
				PropagateMarks_S1(
					id_max[i_dev][L], n_maxcblocks,
					cblock_ID_ref[i_dev], cblock_ID_nbr[i_dev], cblock_ID_nbr_child[i_dev]
				);
				PropagateMarks_S2(
					id_max[i_dev][L], n_maxcblocks,
					cblock_ID_ref[i_dev], cblock_ID_nbr[i_dev], cblock_ID_nbr_child[i_dev]
