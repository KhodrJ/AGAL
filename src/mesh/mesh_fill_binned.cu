...
)
{
	...
	for (int k = 0; k < M_LBLOCK; k += 1)
	{
		...
		if (i_kap_b > -1)
		{
			...
			//for (int l = bin_id_x; l < G_BIN_DENSITY; l++)
			{
				// global_bin_id = l + G_BIN_DENSITY*bin_id_y + G_BIN_DENSITY*G_BIN_DENSITY*bin_id_z;
...
// 							}
			...
			//	cblock_ID_mask[i_kap_b] = -1;
			if (threadIdx.x == 0 && s_D[threadIdx.x]==M_TBLOCK)
				cblock_ID_mask[i_kap_b] = -2;
			...
			if (threadIdx.x == 0)
			{
				if (eligible && !hit_max && s_D[threadIdx.x]>0 && s_D[threadIdx.x]<M_TBLOCK)
...
template <typename ufloat_t, typename ufloat_g_t, const ArgsPack *AP>
int Mesh<ufloat_t,ufloat_g_t,AP>::M_Geometry_FillBinned(int i_dev, int L)
{
	if (n_ids[i_dev][L] > 0)
	{
		Cu_FillBinned<ufloat_t,ufloat_g_t,AP> <<<(M_LBLOCK+n_ids[i_dev][L]-1)/M_LBLOCK,M_TBLOCK,0,streams[i_dev]>>>(
			n_ids[i_dev][L], &c_id_set[i_dev][L*n_maxcblocks], n_maxcblocks, dxf_vec[L], L==MAX_LEVELS-1,
			...
			geometry->c_binned_face_ids_n[i_dev], geometry->c_binned_face_ids_N[i_dev], geometry->c_binned_face_ids[i_dev], geometry->G_BIN_DENSITY
		);
